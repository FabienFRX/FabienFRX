...
export class MockDataService {
  private availableServices: { [key: string]: Service } = {
    gmail: {
      name: 'Gmail',
      icon: '📧',
      actions: [
  ...
  getServiceActions(): any[] {
    return Object.entries(this.availableServices).flatMap(([serviceKey, service]) =>
  ...
  getServiceReactions(): any[] {
    return Object.values(this.availableServices).flatMap(service =>
      service.reactions
    );
  }
}