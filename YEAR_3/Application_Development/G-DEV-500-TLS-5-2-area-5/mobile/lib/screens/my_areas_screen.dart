...
class _MyAreasScreenState extends State<MyAreasScreen> {
  ...
  String _executorStatus = 'unknown'; // running, stopped, unknown
  bool _executorLoading = false;
  ...
  Future<void> _loadExecutorStatus() async {
    try {
      final serverProvider =
          Provider.of<ServerProvider>(context, listen: false);
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      ...
      final response = await apiService.getExecutorStatus(authProvider.token!);
      if (mounted) {
        setState(() {
          _executorStatus = response['status'] ?? 'unknown';
        });
      }
    } catch (e) {
      // Sielntly fail or log, as it's optional status
      print('Failed to load executor status: $e');
    }
  }

  Future<void> _toggleExecutor() async {
    setState(() => _executorLoading = true);
    try {
      final serverProvider =
          Provider.of<ServerProvider>(context, listen: false);
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      ...
      if (_executorStatus == 'running') {
        await apiService.stopExecutor(authProvider.token!);
        setState(() => _executorStatus = 'stopped');
      } else {
        await apiService.startExecutor(authProvider.token!);
        setState(() => _executorStatus = 'running');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to toggle executor: $e')),
        );
      }
    } finally {
      if (mounted) {
        setState(() => _executorLoading = false);
      }
    }
  }
                    ...
                    child: Row(
                      children: [
                        ...
                        if (_executorLoading)
                          ...
                          Switch(
                            value: _executorStatus == 'running',
                            onChanged: (value) => _toggleExecutor(),
