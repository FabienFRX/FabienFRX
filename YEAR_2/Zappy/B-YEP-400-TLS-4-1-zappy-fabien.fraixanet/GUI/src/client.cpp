...
bool Client::connectToServer(const std::string& host, int port)
{
    ...
    std::string mct = "mct\n";
    write(sockfd, mct.c_str(), mct.size());
    return true;
}
...
void Client::parseCommand(const std::string& command, Map& map)
{
    ...
    } else if (cmd_type == "bct") {
        int x, y, q[7];
        if (iss >> x >> y >> q[0] >> q[1] >> q[2] >> q[3] >> q[4] >> q[5] >> q[6]) {
            std::vector<int> quantities(q, q + 7);
            map.setResourcesAt(x, y, quantities);
        }
    } else if (cmd_type == "pdi") {
        std::string id_str;
        if (iss >> id_str && id_str[0] == '#') {
            int id = std::stoi(id_str.substr(1));
            map.removePlayer(id);
        }
    }
}
