...
void handle_eject(server_t *server, int client_index)
{
    ...
    for (i = 0; i < server->player_count; i++) {
        ...
        if (!other || other->is_dead || other->id == player->id ||
        ...
        switch (player->orientation) {
            case NORTH:
                new_y = (new_y - 1 + server->map->height) % server->map->height;
                break;
            case EAST:
                new_x = (new_x + 1) % server->map->width;
                break;
            case SOUTH:
                new_y = (new_y + 1) % server->map->height;
                break;
            case WEST:
                new_x = (new_x - 1 + server->map->width) % server->map->width;
                break;
        }
        ...
        for (int j = 1; j <= server->client_count; j++) {
            if (server->clients[j].fd == other->id) {
                dprintf(server->clients[j].fd, "eject: %d\n", 
                       (player->orientation + 1) % 4 + 1);
                break;
            }
        }
    }
