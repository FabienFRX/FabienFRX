...
void CommandParser::initCommandMap(std::string& currentDir, const std::string& rootDir, int clientSocket, ClientHandler& handler) {
    commandMap = {
        ...
        {"LIST", [this, &currentDir, &rootDir, clientSocket](const std::vector<std::string>& args) {
            return handleLIST(args, currentDir, rootDir, clientSocket);
        }},
        {"RETR", [this, &currentDir, &rootDir, clientSocket](const std::vector<std::string>& args) {
            return handleRETR(args, clientSocket, currentDir, rootDir);
        }},
        {"STOR", [this, &currentDir, &rootDir, clientSocket](const std::vector<std::string>& args) {
            return handleSTOR(args, clientSocket, currentDir, rootDir);
        }},
        ...
        {"PASV", [this, clientSocket](const std::vector<std::string>&) {
            return handlePASVString(clientSocket);
        }},
...
std::string CommandParser::handlePASVString(int clientSocket) {
...
std::string CommandParser::handleLIST(const std::vector<std::string>& args, const std::string& currentDir, const std::string& rootDir, int clientSocket) {
    try {
        std::filesystem::path root = rootDir;
        ...
        send(clientSocket, "451 Requested action aborted.\r\n", 31, 0);
        return "451 Requested action aborted.\r\n";
    }
}

std::string CommandParser::handleRETR(const std::vector<std::string>& args, int clientSocket, const std::string& currentDir, const std::string& rootDir) {
    if (args.size() < 2)
    ...
    try {
        ...
        pid_t pid = fork();
        if (pid == -1) {
            return "550 Failed to fork process.\r\n";
        }
        if (pid == 0) {
            send(clientSocket, "150 Opening data connection.\r\n", 30, 0);
            std::string result = DataTransfer::sendFile(clientSocket, canonicalFile.string());
...
std::string CommandParser::handleSTOR(const std::vector<std::string>& args, int clientSocket, const std::string& currentDir, const std::string& rootDir) {
    if (args.size() < 2)
        return "501 Syntax error in parameters.\r\n";
    try {
        std::filesystem::path root = rootDir;
        ...
        if (pid == 0) {
            send(clientSocket, "150 Opening data connection.\r\n", 30, 0);
            std::string result = DataTransfer::receiveFile(clientSocket, canonicalFile.string());
            send(clientSocket, result.c_str(), result.size(), 0);
            exit(0);
        } else {
            int status;
