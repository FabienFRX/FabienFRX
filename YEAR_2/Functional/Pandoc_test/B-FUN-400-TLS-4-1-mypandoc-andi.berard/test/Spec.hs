...
main = hspec $ do
  ...
  describe "Simple Document Parsing" $ do
    ...
    it "can parse a simple JSON document" $ do
      let input = "{\"header\":{\"title\":\"Simple example\"},\"body\":[{\"type\":\"paragraph\",\"content\":[{\"type\":\"text\",\"content\":\"This is a simple example\"}]}]}"
      let expected = Document (Header "Simple example" Nothing Nothing) [Paragraph [Text "This is a simple example"]]
  ...
  describe "Document Printing" $ do
    ...
    it "can convert Document to JSON" $ do
      ...
      json `shouldContain` "\"content\": \"This is a simple example\""
  ...
  describe "Complex Document Elements" $ do
    it "can parse and print formatted text" $ do
      ...
      -- Test JSON output
      let json = documentToJSON doc
      json `shouldContain` "\"type\": \"bold\""
      json `shouldContain` "\"type\": \"italic\""
    ...
    it "can parse and print links and images" $ do
      ...
      -- Test JSON output
      let json = documentToJSON doc
      json `shouldContain` "\"type\": \"link\""
      json `shouldContain` "\"text\": \"link\""
      json `shouldContain` "\"url\": \"https://example.com\""
      json `shouldContain` "\"type\": \"image\""
      json `shouldContain` "\"alt\": \"picture\""
      json `shouldContain` "\"url\": \"image.jpg\""
